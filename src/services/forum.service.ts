...
const FORUM_STORAGE_KEY = 'yume_tv_forum_posts';
...
export class ForumService {
  ...
  constructor() {
    const storedPosts = localStorage.getItem(FORUM_STORAGE_KEY);
    if (storedPosts) {
      this.posts.set(JSON.parse(storedPosts, (key, value) => {
      ...
      this.posts.set(this.getInitialPosts());
    }

    effect(() => {
      localStorage.setItem(FORUM_STORAGE_KEY, JSON.stringify(this.posts()));
    });
  }

  private getInitialPosts(): ForumPost[] {
    return [];
  }
